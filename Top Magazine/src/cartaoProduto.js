...
export function renderizarCatalago() {
    for (const produtoCatalogo of catalogo) {
        const cartaoProduto = `<div class="border-solid w-48 m-2 flex flex-col p-2 justify-between group shadow-xl shadow-slate-400 rounded-lg group ${produtoCatalogo.acessorio ? "acessorio" : "notebook"}" id="card-produto-${produtoCatalogo.id}">
